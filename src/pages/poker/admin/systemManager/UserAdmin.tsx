...
import { userMangingGet } from '../../../../utils/functions/admin/UserManaging/UserMangingGet'
import { AxiosResponse } from 'axios';
import Swal from 'sweetalert2';
import clsx from 'clsx'
import jwt_decode from 'jwt-decode';
import { roleGet } from '../../../../utils/functions/admin/UserManaging/RoleMangingGet'
import { userRoleChange } from '../../../../utils/functions/admin/UserManaging/UserMangingGet'
...
const Contacts = () => {
    ...
    useEffect(() => {
        async function fetchMyAPI() {
            const data = {}
            const response: AxiosResponse<any, any> = await userMangingGet(data)
            setBufferList(response)
        }
        fetchMyAPI()
    }, [])
    ...
    const setCurrentOption = async (id: any, role: any) => {
        ...
        if (role !== undefined) {
            ...
            const result = await userRoleChange(data)
            setBufferList(result)
        }
    }
    ...
    return (
        <div>
            <div className="flex items-center justify-between flex-wrap gap-4">
                <h2 className="text-xl">User Manager</h2>
                ...
                <div className="mt-5 panel p-0 border-0 overflow-hidden">
                    <div className="table-responsive">
                        <table className="table-striped table-hover">
                            ...
                            <tbody>
                                {bufferList.map((contact: any) => {
                                    return (
                                        <tr key={contact._id}>
                                            <td>
                                                <div className="flex items-center w-max">
                                                    {contact.socialAvatar && (
                                                        <div className="w-max">
                                                            <img src={contact.socialAvatar} className="h-8 w-8 rounded-full object-cover ltr:mr-2 rtl:ml-2" alt="avatar" />
                                                        </div>
                                                    )}
                                                    ...
                                                    {!contact.socialAvatar && !contact.name && (
                                            ...
                                            <td>
                                                {roleList.map((item: any, index: any) =>
                                                    <span
                                                        key={index}
                                                        className={clsx("badge mr-[2px] cursor-pointer", contact.role[0] && contact.role[0].roleName === item.roleName ? "bg-primary" : "bg-dark")}
                                                        onClick={() => setCurrentOption(contact._id, contact.role[0] && contact.role[0].roleName === item.roleName ? undefined : item._id)}
                                                    >
                                                        {item.roleName}
                                                    </span>
                                                )}
                                            ...
                                            <td>{contact.user.createdAt.toString().slice(0, 10).split("-").join("/")}</td>
                                            <td>
            ...
            {value === 'grid' && (
                <div className="grid 2xl:grid-cols-4 xl:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-6 mt-5 w-full">
                    {bufferList.map((contact: any) => {
                        return (
                            <div className="bg-white dark:bg-[#1c232f] rounded-md overflow-hidden text-center shadow relative" key={contact._id}>
                                <div className="bg-white dark:bg-[#1c232f] rounded-md overflow-hidden text-center shadow relative">
                                    ...
                                    >
                                        <img className="object-contain w-4/5 max-h-40 mx-auto" src={contact.socialAvatar} alt="contact_image" />
                                    </div>
                                    <div className="px-6 pb-24 -mt-10 relative">
                                        <div className="shadow-md bg-white dark:bg-gray-900 rounded-md px-2 py-4">
                                            <div className="text-xl">{contact.firstName + contact.lastName}</div>
                                        ...
                                        <div className="mt-6 grid grid-cols-1 gap-4 ltr:text-left rtl:text-right">
                                            <div className="flex items-center">
                                                ...
                                                <div className="truncate text-white-dark">{contact.user.email}</div>
                                            </div>
                                            <div className="flex items-center">
                                    ...
                                    <div className="mt-6 flex gap-4 absolute bottom-0 w-full ltr:left-0 rtl:right-0 p-6">
                                        ...
                                        <button type="button" className="btn btn-outline-danger w-full" onClick={() => deleteUser(contact)}>
                                            Delete
                                        </button>
                                    </div>
                                </div>
                                        ...
                                        <form>
                                            <div className='flex justify-between items-center'>
                                                <div className="mb-5">
                                                    ...
                                                    <input id="First Name" type="text" placeholder="Enter Name" className="form-input" value={params.name} onChange={(e) => changeValue(e)} />
                                                </div>
                                                <div className="mb-5">
                                                    <label htmlFor="email">First Name</label>
                                                    <input id="First Name" type="email" placeholder="Enter Email" className="form-input" value={params.email} onChange={(e) => changeValue(e)} />
                                                </div>
                                            </div>
                                            <div className="mb-5">
